...
import {
  Form, Input, Radio, Button, Divider, Typography,
  Row, Col, Card, Image, message,
} from "antd";
...
const CheckoutPage = () => {
  ...
  const handleOrder = async () => {
    ...
    if (quantity > product.stock) { message.error(`Only ${product.stock} item(s) in stock`); return; }
    try {
      const values = await form.validateFields();
      await placeOrder({ productId: product._id, quantity, paymentMode, deliveryDetails: values });
      setOrdered(true);
    } catch (error) {
      message.error(error.response?.data?.message || "Order failed");
    }
  };
      ...
      <div style={{ minHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
        <div style={{ textAlign: "center", maxWidth: 420 }}>
          ...
          <Text type="secondary" style={{ fontSize: 16 }}>
            Your order for <strong>{quantity} × {product.name}</strong> has been placed successfully.
          </Text>
          <br /><br />
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} md={15}>
          <Form form={form} layout="vertical" requiredMark="optional">
            ...
            <Form.Item label="Mobile Number" name="mobile" rules={[{ required: true, message: "Please enter your mobile number" }, { pattern: /^[6-9]\d{9}$/, message: "Enter a valid 10-digit Indian mobile number" }]}>
            ...
            {paymentMode === "upi" && (
              <Card style={{ marginBottom: 16, background: "#f9f9ff" }}>
                <Text type="secondary" style={{ display: "block", marginBottom: 12 }}>
                  Scan the QR code below to pay ₹ {(product.price * quantity).toLocaleString()}
                </Text>
                <Image src={UPI_QR_IMAGE} alt="UPI QR Code" width={180} style={{ borderRadius: 8, display: "block", margin: "0 auto" }} />
            ...
            <Button type="primary" size="large" block onClick={handleOrder} style={{ marginTop: 8 }}>Place Order</Button>
          </Form>
        </Col>
        <Col xs={24} md={9}>
          <Card style={{ position: "sticky", top: 80 }} title="Order Summary">
            {product.image && (
              <img src={product.image} alt={product.name} style={{ width: "100%", height: 160, objectFit: "cover", borderRadius: 8, marginBottom: 16 }} />
            )}
            <Title level={5} style={{ marginBottom: 4 }}>{product.name}</Title>
            <Text type="secondary" style={{ fontSize: 13 }}>{product.brand} · {product.category}</Text>
            <Divider style={{ margin: "12px 0" }} />
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <Text type="secondary">Price per item</Text>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <Text strong style={{ fontSize: 18, color: "#ff6a00" }}>
